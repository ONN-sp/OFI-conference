...
with tf.device('/gpu:0'):
    ...
    # plt.imshow(img)
    # plt.show() # 必须要有,否则显示不出来

    # def apply(img, method):
    #     Y = method(img)
    #     plt.imshow(Y)
    #     plt.show()
    ...
    # # 随机裁剪(随机位置),裁剪为指定大小
    # crop_size = 100 # 像素
    # Y = tf.image.random_crop(img, (crop_size, crop_size, 3))
    # plt.imshow(Y)
    # plt.show()
    # # 变化亮度
    # max_delta = 0.5 # 在[-max_delta, max_delta]区间上随机增加亮度
    # Y = tf.image.random_brightness(img, max_delta)
    # plt.imshow(Y)
    # plt.show()
    # # 变化色调
    # max_delta = 0.5 # 在[-max_delta, max_delta]区间上随机增加色调
    # Y = tf.image.random_hue(img, max_delta)
    # plt.imshow(Y)
    # plt.show()


    # 下面利用ResNet18训练增广后的数据
    ...
    class Printcor(tf.keras.callbacks.Callback): # 一个epoch随机生成一个不同亮度训练集,那么可以增加图像的多样性,也可以理解为增加训练集数量
        def on_epoch_begin(self, epoch, logs):
            global x_train_new # 声明它为全局变量
            max_delta = 0.5 # 在[-max_delta, max_delta]区间上随机增加亮度
            x_train_new = [tf.image.random_brightness(i, max_delta) for i in x_train]
            print(type(x_train_new))
            x_train_new = np.array(x_train_new)
    x_train_new = x_train
    model.fit(x_train_new, y_train, batch_size = 64, epochs = 5, callbacks = [Printcor()])
